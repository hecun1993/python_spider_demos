...
def parse(param):
    ...
    try:
        ...
        if comments_count > 0:
            ...
            person = result.select('.about_news')[0].text.strip().replace('\n', ' ').replace('\xa0', ' ') if result.select('.about_news') else None
            time = result.select('.about_news')[1].text.strip().replace('\n', ' ').replace('\xa0', ' ') if result.select('.about_news') else None
            ...
            save_to_mongo({'title': title, 'person': person, 'time': time, 'content': content,
                 'column': column,
                 'dz': dz, 'url': param, 'comments': comments})
        else:
            title = result.title.text if result.title else None
...
def write_to_file(content):
    with open('paper_news_result_01.csv', 'a', encoding='utf-8') as f:
